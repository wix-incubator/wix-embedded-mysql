  ...
  "EmbeddedMysql download config" should {
    ...
    "store download cache in custom location" in {
      withTempDir { tempDir =>
        ...
        val mysqld = start(anEmbeddedMysql(testVersion, downloadConfig))
    ...
    "uses custom download base url" in {
      withTempDir { tempDir =>
        ...
        start(anEmbeddedMysql(testVersion, downloadConfig)) must throwA[DistributionException].like {
    ...
    "downloads via custom download base url" in new context {
      ensureVersionPresentInCache(testVersion)
      ...
      withTempDir { tempDir =>
        ...
        start(anEmbeddedMysql(testVersion, downloadConfig))
